...
def hello_world() :
    ...
    output += "<h4>This is a stateless server. All the keys (including CA-private) "
    output += "must be provided by the caller for each operation.</h4><hr/>\n"

    result = handle_form_request(request)
    output += result
    if result : output += "<hr/>\n"

    output += genform_keygen()
    output += genform_certsign()
...
def keygen_handler() :
    ...
    if (oscme_in.major()) :
        if (__debug > 3) : print("oscme_check(major): " + oscme_in.detailed())
        ...
        return result
    elif oscme_in.errors() and (__debug > 4) :
    ...
    if (__debug > 4) : print(result)

    return jsonify(result)
    # keygen_handler ends

@app.route('/certsign', methods = ['GET', 'POST'])
def certsign_handler() :
